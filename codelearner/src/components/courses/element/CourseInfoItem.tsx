...
import { imageCourse } from "../../../service/helper/image_update/imageCourse";
...
const CourseInfoItem: React.FC<{
  ...
  const [formData, setFormData] = useState<Omit<Course, "created_at" | "logo">>(
    {
      id: course.id,
      name: course.name,
  ...
  return (
    <>
      ...
      <div className="mt-3 ">
        <ImageSelector
          defaultImage={course.logo!}
          updateFunc={imageCourse}
          owner_id={course.id.toString()}
          token={token!}
        />
      </div>
    </>
